...
const Frame7 = () => {
  ...
  useEffect(() => {
    const fetchProductIds = async () => {
      try {
        const response = await axios.get("http://106.52.158.123:5000/api/product_ids");
        setProductIds(response.data.map(item => item.id.toString())); // 确保数据是字符串数组
      } catch (error) {
  ...
  const handleNextPage = () => {
    if (currentPage < Math.ceil(productIds.length / itemsPerPage)) {
  ...
  const currentItems = productIds.slice(startIndex, startIndex + itemsPerPage);
        ...
        <section className="self-stretch flex flex-col items-center justify-start gap-6 max-w-full">
          <div className="grid-container">
            {currentItems.map((id, index) => (
              <ProductCard key={index} productId={id} propWidth="100%" propMinWidth="200px" />
            ))}
          ...
          <div className="pagination-container">
            ...
            <span>
              {currentPage} / {Math.ceil(productIds.length / itemsPerPage)}
            </span>
            <Button
              variant="contained"
              onClick={handleNextPage}
              disabled={currentPage === Math.ceil(productIds.length / itemsPerPage)}
            >
              下一页
            </Button>
          </div>
